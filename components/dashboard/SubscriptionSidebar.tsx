...
export default function SubscriptionSidebar({ active = "billing", scannerCount, user }: { active?: "billing" | "web-scanner" | "os-scanner" | "api-scanner" | "hybrid-scanner" | "network-overview" | "network-inventory" | "network-agents" | "ai-overview" | "ai-launch" | "ai-pipeline" | "ai-knowledge" | "ai-reports" | "report-centre"; scannerCount?: number; user?: CurrentUserResponse }) {
  ...
  return <aside className={`dashboard-sidebar subscription-sidebar${active.startsWith("ai-") ? " ai-module-sidebar" : ""}${collapsed ? " subscription-sidebar-collapsed" : ""}`}>
    ...
    <nav className="dashboard-navigation" aria-label="Workspace navigation">
      ...
      <MenuGroup activeItem={active === "web-scanner" ? "Web Scanner" : active === "os-scanner" ? "OS Scanner" : active === "api-scanner" ? "API Scanner" : active === "hybrid-scanner" ? "Hybrid Scanner" : undefined} badge={["web-scanner", "os-scanner", "api-scanner", "hybrid-scanner"].includes(active) ? String(scannerCount ?? 0) : "4"} icon="scanner" items={[{ href: "/configuration", label: "Web Scanner" }, { href: "/os-scanner", label: "OS Scanner" }, { href: "/hybrid-scanner", label: "Hybrid Scanner" }, { href: "/api-scanner", label: "API Scanner" }]} label="Scanners" />
